...
info_cols = ['id', 'Sequence', 'Uniprot', 'Gene names', 'Protein names', 'Amino acid before', 'Last amino acid', 'Length', 'Start position', 'End position', 'Missed cleavages', "Scalar", "Ctrl missingness", "Treat missingness", "Total missingness", "Control", "Treat", "Treat/Control", "Treat/Control_pval", "Treat/Control_adj-p", "Sig Pept num"]
```
...
if ProteinID_col_pept is not None and ProteinID_col_prot is not None and PeptCounts_col is not None:
    ...
    raise NotImplementedError("The error was triggered because either the search tool is not specified or not columns specification are not provided. Please specify the search tool or provide the columns for protein IDs, peptides as well as peptide counts.")
```


```{python}
## Need to implement checking the quantification columns!!!
...
if search_tool.lower() == "maxquant":
    ...
    warnings.warn("Please specify the search tool or user need to filter the data by themselves. From here forward, data are treated as filtered already!", UserWarning)
```
...
```{python}
barcode_dir = result_dir + sample + "_barcodes/"
Path(barcode_dir).mkdir(parents=True, exist_ok=True)
intensity_cols = double_int_cols
all_lytic_sites = []
sig_lytic_sites = []
semi_only = False
...
for uniprot_id in all_uniprot_ids:
    ...
    if len(prot_seq_search) > 0:
        ...
        protein["Lytic site"] = lyticsites
        pept_num = len(clean_pepts)
        sig_num = sum(1 for i in protein["Treat/Control_pval"] if i < sig_thr)
        semi_num = protein["pept_type"].to_list().count("Semi-tryptic")
        non_num = protein["pept_type"].to_list().count("Non-tryptic")
        prok_num = protein["pept_type"].to_list().count("Semi-tryptic") + protein["pept_type"].to_list().count("Non-tryptic")

        if semi_only:
            # Semi-tryptic peptide sites
            protein2explode = protein[protein["pept_type"] == "Semi-tryptic"].copy()
        else:
            protein2explode = protein.copy()

        if protein2explode.shape[0] > 0:
            protein_lys = protein2explode.explode("Lytic site")
            protein_lys.reset_index(drop=True, inplace=True)
            protein_lys.index = protein_lys["Lytic site"]
            # protein_lys[intensity_cols] = protein_lys[intensity_cols].replace(0, np.nan)
            protein_lys[intensity_cols] = 2 ** (protein_lys[intensity_cols])
            protein_lys_grouped = protein_lys[intensity_cols].groupby("Lytic site").sum(min_count=1)
            protein_lys_grouped = np.log2(protein_lys_grouped)
            # protein_lys_grouped = protein_lys_grouped.fillna(0)

            protein_lys_grouped["Uniprot"] = uniprot_id
            protein_lys_grouped["Site"] = protein_lys_grouped.index.to_list()
            protein_lys_grouped.index = uniprot_id + '@' + protein_lys_grouped["Site"]
            protein_lys_grouped["Pos"] = [int(re.sub(r"\D", "", site)) for site in protein_lys_grouped["Site"]]
            protein_lys_grouped["Lytic site type"] = ["Trypsin" if 'K' in i or 'R' in i else "ProK" for i in protein_lys_grouped.index]
            protein_lys_grouped["Pept num"] = pept_num
            protein_lys_grouped["Sig pept num"] = sig_num
            protein_lys_grouped["Partial tryptic num"] = semi_num
            protein_lys_grouped["Non-tryptic num"] = non_num
            protein_lys_grouped["ProK num"] = prok_num
            protein_lys_grouped["Ctrl missingness"] = protein_lys_grouped[double_ctrl_cols].isna().sum(axis=1)
            protein_lys_grouped["Treat missingness"] = protein_lys_grouped[double_treat_cols].isna().sum(axis=1)
            protein_lys_grouped["Total missingness"] = protein_lys_grouped["Ctrl missingness"] + protein_lys_grouped["Treat missingness"]
            protein_lys_grouped = protein_lys_grouped[protein_lys_grouped["Total missingness"] < len(intensity_cols)]
            protein_lys_grouped["Control"] = protein_lys_grouped[double_ctrl_cols].mean(axis=1).fillna(0)
            protein_lys_grouped["Treat"] = protein_lys_grouped[double_treat_cols].mean(axis=1).fillna(0)
            protein_lys_grouped["Treat/Control"] = protein_lys_grouped["Treat"] - protein_lys_grouped["Control"]
            protein_lys_grouped["Treat/Control_pval"] = sp.stats.ttest_ind(protein_lys_grouped[double_treat_cols], protein_lys_grouped[double_ctrl_cols], axis=1, nan_policy='omit').pvalue
            protein_lys_grouped["Treat/Control_pval"].fillna(0, inplace=True)
            protein_lys_grouped["Treat/Control_adj-p"] = sp.stats.false_discovery_control(protein_lys_grouped["Treat/Control_pval"].fillna(0))
            protein_lys_grouped["Treat/Control_adj-p_BY"] = sp.stats.false_discovery_control(protein_lys_grouped["Treat/Control_pval"].fillna(0), method = 'by')
            protein_lys_grouped["Treat/Control_adj-p_bonferroni"] = multitest.multipletests(protein_lys_grouped["Treat/Control_pval"].fillna(0), method='bonferroni')[1]
            protein_lys_grouped_sig = protein_lys_grouped[protein_lys_grouped["Treat/Control_pval"] < sig_thr].copy()
            protein_lys_grouped["Sig site num"] = protein_lys_grouped_sig.shape[0]
            sig_prok_num = protein_lys_grouped_sig[protein_lys_grouped_sig["Lytic site type"] == "ProK"].shape[0]
            protein_lys_grouped["Sig ProK num"] = sig_prok_num
            protein_lys_grouped_sig["Sig ProK num"] = sig_prok_num

            protein_lys_grouped.sort_values(by=["Pos"], inplace=True)
            protein_lys_grouped = protein_lys_grouped[protein_lys_grouped.columns.to_list()[-18:] + protein_lys_grouped.columns.to_list()[:-18]]
            protein_lys_grouped_sig = protein_lys_grouped_sig[protein_lys_grouped_sig.columns.to_list()[-17:] + protein_lys_grouped_sig.columns.to_list()[:-17]]
            all_lytic_sites.append(protein_lys_grouped)
            sig_lytic_sites.append(protein_lys_grouped_sig)

            if uniprot_id in double_sig_uniprot_ids:
                protein_lys_prok = protein_lys_grouped[protein_lys_grouped["Lytic site type"] == "ProK"].copy()
                lys_fc_diff = protein_lys_prok[["Site", "Pos", "Treat/Control", "Treat/Control_pval"]].copy()
                lys_fc_diff.index = lys_fc_diff["Site"]
                lys_fc_diff_max = lys_fc_diff["Treat/Control"].abs().max()
                fc_diff_names = [aa + str(i+1) for i, aa in enumerate(list(sequence))]
                bar_code = [(1.0,1.0,1.0)] * len(fc_diff_names)
                for i in range(lys_fc_diff.shape[0]):
                    if lys_fc_diff.iloc[i, 2] > 0:
                        if lys_fc_diff.iloc[i, 3] < 0.05:
                            bar_code[lys_fc_diff.iloc[i, 1] - 1] = up_pal_vals[np.ceil(min(abs(lys_fc_diff.iloc[i, 2]),3.322)/lys_fc_diff_max * color_levels).astype("int") - 1]
                        else:
                            bar_code[lys_fc_diff.iloc[i, 1] - 1] = insig_pal_vals[np.ceil(min(abs(lys_fc_diff.iloc[i, 2]),3.322)/lys_fc_diff_max * color_levels).astype("int") - 1]
                    else:
                        if lys_fc_diff.iloc[i, 3] < 0.05:
                            bar_code[lys_fc_diff.iloc[i, 1] - 1] = down_pal_vals[np.ceil(min(abs(lys_fc_diff.iloc[i, 2]),3.322)/lys_fc_diff_max * color_levels).astype("int") - 1]
                        else:
                            bar_code[lys_fc_diff.iloc[i, 1] - 1] = insig_pal_vals[np.ceil(min(abs(lys_fc_diff.iloc[i, 2]),3.322)/lys_fc_diff_max * color_levels).astype("int") - 1]

                # plot_barcode(bar_code, barcode_name = uniprot_id, ticklabel=[aa[0] for aa in lys_fc_diff["lys"]], size = (lys_fc_diff.shape[0]//10,2))
                # plot_barcode(bar_code, barcode_name = uniprot_id, ticklabel=[aa[0] for aa in lys_fc_diff["lys"]], size = (10,2))
                # plt.tight_layout()
                # plt.savefig(barcode_dir + uniprot_id + "_protKsite_all_nosep_barcode_wt_filter_small.pdf", dpi=100)
                # plt.close()
                plot_barcode(bar_code, barcode_name = uniprot_id, ticklabel=[fc_diff_names[j] for j in np.arange(0,seq_len,np.ceil(seq_len/10).astype("int"))], size = (min(max(math.floor(seq_len/3), 10), 50), min(max(math.floor(protein_lys_prok.shape[0]/5), 6),30)))
                plt.tight_layout()
                if semi_only:
                    plt.savefig(barcode_dir + uniprot_id.replace('|','-') + "_protKsite_semi_nosep_barcode_wt_filter.pdf", dpi=100)
                else:
                    plt.savefig(barcode_dir + uniprot_id.replace('|','-') + "_protKsite_all_nosep_barcode_wt_filter.pdf", dpi=100)
                plt.close()

if semi_only:
    proteome_lytic_sites = pd.concat(all_lytic_sites)
    proteome_lytic_sites.to_csv(result_dir + sample + "_proteome_semi_sites.tsv", sep='\t')
    proteome_lytic_sites.to_pickle(result_dir + sample + "_proteome_semi_sites.pkl")

    proteome_lytic_sites_sig = pd.concat(sig_lytic_sites)
    proteome_lytic_sites_sig.to_csv(result_dir + sample + "_proteome_semi_sites_sig.tsv", sep='\t')
    proteome_lytic_sites_sig.to_pickle(result_dir + sample + "_proteome_semi_sites_sig.pkl")
else:
    proteome_lytic_sites = pd.concat(all_lytic_sites)
    proteome_lytic_sites.to_csv(result_dir + sample + "_proteome_lytic_sites.tsv", sep='\t')
    proteome_lytic_sites.to_pickle(result_dir + sample + "_proteome_lytic_sites.pkl")

    proteome_lytic_sites_sig = pd.concat(sig_lytic_sites)
    proteome_lytic_sites_sig.to_csv(result_dir + sample + "_proteome_lytic_sites_sig.tsv", sep='\t')
    proteome_lytic_sites_sig.to_pickle(result_dir + sample + "_proteome_lytic_sites_sig.pkl")
```
    ...
    if len(prot_seq_search) > 0:
        ...
        if protein2explode.shape[0] > 0:
            ...
            if uniprot_id in double_sig_uniprot_ids:
                ...
                plt.tight_layout()
                if semi_only:
                    plt.savefig(barcode_dir + uniprot_id.replace('|','-') + "_sig_protKsite_semi_nosep_barcode_wt_filter.pdf", dpi=100)
                else:
                    plt.savefig(barcode_dir + uniprot_id.replace('|','-') + "_sig_protKsite_all_nosep_barcode_wt_filter.pdf", dpi=100)
                plt.close()

if semi_only:
    proteome_lytic_sites = pd.concat(all_lytic_sites)
    proteome_lytic_sites.to_csv(result_dir + sample + "_sig_proteome_semi_sites.tsv", sep='\t')
    proteome_lytic_sites.to_pickle(result_dir + sample + "_sig_proteome_semi_sites.pkl")

    proteome_lytic_sites_sig = pd.concat(sig_lytic_sites)
    proteome_lytic_sites_sig.to_csv(result_dir + sample + "_sig_proteome_semi_sites_sig.tsv", sep='\t')
    proteome_lytic_sites_sig.to_pickle(result_dir + sample + "_sig_proteome_semi_sites_sig.pkl")
else:
    proteome_lytic_sites = pd.concat(all_lytic_sites)
    proteome_lytic_sites.to_csv(result_dir + sample + "_sig_proteome_lytic_sites.tsv", sep='\t')
    proteome_lytic_sites.to_pickle(result_dir + sample + "_sig_proteome_lytic_sites.pkl")

    proteome_lytic_sites_sig = pd.concat(sig_lytic_sites)
    proteome_lytic_sites_sig.to_csv(result_dir + sample + "_sig_proteome_lytic_sites_sig.tsv", sep='\t')
    proteome_lytic_sites_sig.to_pickle(result_dir + sample + "_sig_proteome_lytic_sites_sig.pkl")
```
